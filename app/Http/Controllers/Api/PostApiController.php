...
class PostApiController extends Controller {
    ...
    public function index(Request $request) {
        try {
            // The cache key is generated by hashing the request parameters
            $cacheKey = $this->generateCacheKey('post', 's_list_', $request);

            // Cache::remember checks if the cache entry exists
            // If yes: returns cache data
            // If no: executes the closure, stores the result in the cache and returns it
            return $this->cacheData($cacheKey, function () use ($request) {
                if (Post::count() === 0) {
    ...
    public function store(Request $request): JsonResponse {
        try {
            ...
            // Forget the cache for the posts list
            $this->forgetCacheByModelType('App\Models\Post');
    ...
    public function show(string $id, Request $request): JsonResponse {
        try {
            // The cache key is generated by hashing the request parameters and the post ID
            $cacheKey = $this->generateCacheKey('post', '_' . $id . '_', $request);

            // Cache::remember checks if the cache entry exists
            // If yes: returns cache data
            // If no: executes the closure, stores the result in the cache and returns it
            return $this->cacheData($cacheKey, function () use ($id, $request) {
    ...
    public function update(Request $request, string $id): JsonResponse {
        try {
            ...
            if ($request->user()->id !== $post->user_id && ($request->user()->role === 'admin' || $request->user()->role === 'moderator')) {
                ...
                // Forget the cache for the posts list and the post itself
                $this->forgetCacheByModelType('App\Models\Post');
