        ...
        if ($filterArray) {
            ...
            foreach (array_keys($filterArray) as $key) {
                ...
                foreach ($filterArray[$key] as $value) {
                    if (!preg_match('/^[a-zA-Z0-9_,\.\-:@#& \/\+\*]+$/', $value)) { // allow alphanumeric, underscores, commas, periods, colons, dashes, at-signs (@), spaces , hash (#) , ampersands (&) , slashes (/) and plus signs (+) and asterisks (*)
